...
namespace weatherWindowsFormsApp
{
    public partial class temperature : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            //呼叫Method checkYear()檢查輸入的年份是否合法  如果合法回傳true 不合法回傳false
            yearOk = checkYear();
            //呼叫Mehod checkMonth 輸入的月份是否為1~12的數字   如果是回傳true 不是回傳false    
            monthOk = checkMonth();
            ...
            if (yearOk == true) //輸入年份資料合法
            {
                ...
                {
                    //呼叫Method showData()，將該年該月份的資料，在表單的listbox上顯示出來
                    showData();
            ...
            else//年份輸入不正確
            {
                ...
                inputYearMaskedTextBox.Focus(); //將游標移到年份輸入處
            }
        }
        //defind showData()
        ...
        private void showData()
        {
            ...
            //呼叫Method查核該月份有幾天，回傳該月份的天數
            totalDays = checkDays(); 

            //呼叫Method查出該月最高溫跟最低溫是多少，呼叫Method時，要給兩個參考的變數，存放最高溫及最低溫
            checkTemperature(ref lowBound, ref highBound);

            //呼叫Method將相關的資料在listbox上顯示，呼叫Method時要給三個變數，天數、最低溫度、最高溫度
            displayData(totalDays,lowBound,highBound);
        }//end of showData()

        //定義displayData()，呼叫Method將相關的資料在listbox上顯示，呼叫Method時要給三個變數，天數、最低溫度、最高溫度
        private void displayData(int totalDays,int lowBound,int highBound)
        {
            Random rand = new Random();
            for (int i = 0 ; i < totalDays ; i ++ ) //利用迴圈產生totalDays筆資料
            {
                //將隨機亂數產生的資料存放到listbox中，產生的數值介於lowBound-1~highBound + 1
                TemperatureListBox.Items.Add(rand.Next(lowBound,highBound + 1)); 
                
            }
        }

        //定義checkTemperature()，沒有回傳值，有兩個參考的變數，分別代表最低溫和最高溫度
        ...
        private int checkDays()
        {
            ...
            //呼叫Method isLeapYear()，呼叫的時候，要給一個整數的變數，表示查詢的年份。true的話表示是犉年，false的話表示不是閏年
        ...
        //定義isLeapYear()，回傳該年是否為閏年Method有一個整數的參數即為查核是否為閏年的年份，回傳true為閏年;false則不為閏年
        ...
        //define checkYear()，Method回傳一個布林值，ture表示合法，false表示不合法
        ...
        private void ComputeButton_Click(object sender, EventArgs e)
        {
            ...
            //呼叫Method將listbox資料存放陣列中
            length = readData(temp);

            //呼叫Method找出最高溫度及最高溫日期，回傳最高溫度 呼叫\Method時給int陣列，陣列的使用長度，int陣列，陣列的使用長度，int陣列，陣列的使用長度參考int形式的值
        ...
        //定義displayHighestemperature()，呼叫時要給一個整數，一個整數陣列，一個整數
        ...
        //回傳最高溫度 呼叫Method時給int陣列，陣列的使用長度，int陣列，陣列的使用長度，int陣列，陣列的使用長度參考int形式的值
        ...
        //整數(最高溫)，整數陣列(最高溫的日期)，整數陣列(存放當月每日溫度)，整數(當月存放溫度的日數)，參考整數(最高溫度的日數)
        private int findthesame(int compare,int [] highTemperature, int [] temp,int length,ref int counter)
        {
            ...
            for (int i = 0 ; i < length ; i ++ )
            {
                ...
                {
                    highTemperature[counter] = i + 1; //將最高溫的日期存入陣列中
        ...
        //定義findHighesttemperature()，回傳最高溫度，呼叫Method時要給一個整數的陣列和一個整數
        private int findHighesttemperature(int [] temp,int length)
        {
            int highest = temp[0]; //將陣列中的元素設為最高溫

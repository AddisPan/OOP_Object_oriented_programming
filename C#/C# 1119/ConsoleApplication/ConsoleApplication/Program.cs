...
namespace ConsoleApplication
{
    class Program
    {
        ...
        static void inputTwoIntegers(out int small, out int big)
        {
            ...
            while (int.TryParse(Console.ReadLine(),out small) == false) //當使用者沒輸入整數時再詢問一次
            {
                Console.WriteLine("請輸入整數:");
            }
            Console.WriteLine("第一個整數是" + small.ToString());

            Console.WriteLine("請輸入第二個整數:");
            while (int.TryParse(Console.ReadLine(), out big) == false) //當使用者沒輸入整數時再詢問一次
            {
                Console.WriteLine("請輸入整數:");
            }
            Console.WriteLine("第二個整數是" + big.ToString());
